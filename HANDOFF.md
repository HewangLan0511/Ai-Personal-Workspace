...
| 现在的阶段 | **阶段0 ✅ 通过；阶段1 已交付代码，经 REVIEW-002/003 判定 ❌ 驳回，正在返工**。工作区**已有完整代码**：`core/`（Rust 骨架）、`ui/`（Vue3 前端）、`system/`（Python sidecar）、`tools/gate.py`（门禁）。 |
| 谁负责审核 | 监制「肉编器001号」。**未经审核通过，不得进入下一阶段。** |
| 你现在该做的 | 先读 `docs/reviews/LEDGER.md` 看阶段1 剩余驳回项 → 修完自跑门禁 → 送审（第 8 章） |
...
```
当前任务 = 阶段1 返工（REVIEW-002 / REVIEW-003 驳回项）
指令文件 = docs/agent-dev/04-阶段指令-基础框架.md
遗留清单 = docs/reviews/LEDGER.md「遗留项登记」（L-005 ~ L-016）
送审口令 = 「申请审核阶段 1」
```
...
- 注意：ADR-001（窗口控制与进程启动）= **Rust**，已定稿（`docs/adr/ADR-001`）；
  阶段1 代码已在工作区，当前处于"驳回返工"状态，先看 `docs/reviews/LEDGER.md`。
